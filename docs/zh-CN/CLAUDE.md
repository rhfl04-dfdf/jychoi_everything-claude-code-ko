...
이 파일은 Claude Code(claude.ai/code)가 이 저장소에서 코드 작업 시 안내를 제공합니다.

## 프로젝트 개요

이것은 **Claude Code 플러그인**으로, 프로덕션 수준의 agent, skill, hook, command, rule 및 MCP 설정의 모음입니다. 이 프로젝트는 Claude Code를 사용한 소프트웨어 개발에 검증된 워크플로우를 제공합니다.
