...
@Composable
actual fun PlatformStatusBar(darkIcons: Boolean) {
    // iOS는 UIKit interop 또는 Info.plist를 통해 이를 처리합니다.
}
```
...
LazyColumn {
    items(
        items = items,
        key = { it.id }  // Stable key는 아이템 재사용 및 애니메이션을 가능하게 합니다.
    ) { item ->
        ItemRow(item = item)
    }
}
...
```kotlin
// 나쁨 — 매 recomposition마다 새로운 람다와 리스트 생성
items.filter { it.isActive }.forEach { ActiveItem(it, onClick = { handle(it) }) }

// 좋음 — 각 아이템에 key를 지정하여 콜백이 올바른 행에 유지되도록 함
