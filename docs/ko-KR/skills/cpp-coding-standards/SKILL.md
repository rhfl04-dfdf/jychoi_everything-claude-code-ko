...
void transform(std::string s);               // sink: 값으로 (move 예정)

// F.20 + F.21: 출력 매개변수가 아닌 반환값
struct ParseResult {
    std::string token;
    int position;
};

ParseResult parse(std::string_view input);   // GOOD: struct 반환

// BAD: 출력 매개변수
void parse(std::string_view input,
           std::string& token, int& pos);    // 이렇게 하지 말것
```
...
```cpp
// C.21: 리소스를 관리해야 한다면, 다섯 가지 모두 정의
class Buffer {
public:
...
void run() {
    ...
    // E.17: 여기서 모든 것을 catch하지 않음 -- 예상치 못한 오류는 전파
}
```
...
public:
    ...
    // 변경이 필요할 때만 non-const
    void record(double value) { reading_ = value; }

private:
    const std::string id_;  // Con.4: 생성 후 절대 변경 안 됨
    double reading_{0.0};
};
...
```cpp
// CP.20 + CP.44: RAII 락, 항상 명명
class ThreadSafeQueue {
public:
    void push(int value) {
        std::lock_guard<std::mutex> lock(mutex_);  // CP.44: 명명!
        queue_.push(value);
        cv_.notify_one();
    }
...
```cpp
// Enum.3 + Enum.5: 스코프 열거형, ALL_CAPS 금지
enum class Color { red, green, blue };
enum class LogLevel { debug, info, warning, error };

// BAD: 일반 열거형은 이름을 누출하고, ALL_CAPS는 매크로와 충돌
...
// SF.11: 자기 완결적 -- 이 헤더에 필요한 모든 것을 include
#include <string>
#include <vector>
