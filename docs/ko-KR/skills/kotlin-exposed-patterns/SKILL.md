...
description: DSL 쿼리, DAO 패턴, transaction, HikariCP connection pooling, Flyway migration, repository 패턴을 포함한 JetBrains Exposed ORM 패턴입니다.
origin: ECC
---

# Kotlin Exposed Patterns

JetBrains Exposed ORM을 사용한 데이터베이스 액세스를 위한 포괄적인 패턴으로, DSL 쿼리, DAO, transaction 및 프로덕션 환경에 적합한 구성을 포함합니다.

## When to Use

- Exposed를 사용한 데이터베이스 액세스 설정
- Exposed DSL 또는 DAO를 사용하여 SQL 쿼리 작성
- HikariCP를 사용한 connection pooling 구성
- Flyway를 사용한 데이터베이스 migration 생성
- Exposed를 사용한 repository 패턴 구현
- JSON 컬럼 및 복잡한 쿼리 처리

## How It Works

Exposed는 두 가지 쿼리 스타일을 제공합니다: 직접적인 SQL 유사 표현식을 위한 DSL과 엔티티 수명 주기 관리를 위한 DAO입니다. HikariCP는 `HikariConfig`를 통해 구성된 재사용 가능한 데이터베이스 connection pool을 관리합니다. Flyway는 시작 시 버전이 지정된 SQL migration 스크립트를 실행하여 스키마를 동기화 상태로 유지합니다. 모든 데이터베이스 작업은 coroutine 안전성과 원자성을 위해 `newSuspendedTransaction` 블록 내부에서 실행됩니다. repository 패턴은 Exposed 쿼리를 인터페이스 뒤로 캡슐화하여 비즈니스 로직이 데이터 레이어와 분리되도록 하며, 테스트에서는 인메모리 H2 데이터베이스를 사용할 수 있게 합니다.
...
| `newSuspendedTransaction { }` | Coroutine 지원 transaction 블록 |
| `Table.selectAll().where { }` | 조건이 포함된 쿼리 |
| `Table.insertAndGetId { }` | 삽입 및 생성된 ID 반환 |
| `Table.update({ condition }) { }` | 일치하는 행 업데이트 |
| `Table.deleteWhere { }` | 일치하는 행 삭제 |
| `Table.batchInsert(items) { }` | 효율적인 일괄 삽입 |
| `innerJoin` / `leftJoin` | 테이블 조인 |
| `orderBy` / `limit` / `offset` | 정렬 및 페이징 |
| `count()` / `sum()` / `avg()` | 집계 함수 |

**기억하세요**: 간단한 쿼리에는 DSL 스타일을 사용하고, 엔티티 수명 주기 관리가 필요한 경우에는 DAO 스타일을 사용하세요. Coroutine 지원을 위해 항상 `newSuspendedTransaction`을 사용하고, 테스트 가능성을 위해 데이터베이스 작업을 repository 인터페이스 뒤로 캡슐화하세요.
