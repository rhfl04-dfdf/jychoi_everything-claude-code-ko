...
```python
# force=True는 비디오가 이미 인덱싱된 경우의 에러를 건너뜁니다
...
# search()는 결과를 찾을 수 없을 때 InvalidRequestError를 발생시킵니다.
...
# index_scenes()에는 force 파라미터가 없습니다. 장면 index가 이미 존재하면
# 에러를 발생시킵니다. 에러에서 기존 index ID를 추출하세요.
try:
...
# 관련성이 낮은 노이즈를 필터링하려면 score_threshold를 사용하세요 (권장: 0.3+)
try:
...
# 서버 측에서 해상도, 품질 또는 aspect ratio 변경
...
# 짧은 구간을 reframe하는 것을 항상 선호하세요:
reframed = video.reframe(start=0, end=60, target="vertical", mode=ReframeMode.smart)

# 전체 길이 비디오를 위한 비동기 reframe (None 반환, 결과는 webhook을 통해 전달):
