...
REFACTOR -> 테스트를 통과한 상태를 유지하면서 코드 개선
REPEAT  -> 다음 요구사항으로 계속
```
...
// Step 6: 필요 시 리팩터링, 테스트가 여전히 통과하는지 확인
```
    ...
    test("timeout after delay") {
        runTest {
            ...
            shouldThrow<TimeoutCancellationException> {
                withTimeout(100) {
                    service.slowOperation() // 100ms 이상 소요
                }
            }
        }
    }
...
class FlowServiceTest : FunSpec({
    ...
    test("searchUsers debounces input") {
        runTest {
            ...
            queries.emit("a")
            queries.emit("ab")
            queries.emit("abc") // 이것만 검색을 트리거해야 함
            advanceTimeBy(500)
...
// 참고: 아래의 직렬화 라운드트립 테스트는 User data class에
// @Serializable (kotlinx.serialization) 어노테이션이 필요합니다.
...
kover {
    reports {
        ...
        verify {
            rule {
                minBound(80) // 80% 커버리지 미만 시 빌드 실패
            }
        }
    }
}
