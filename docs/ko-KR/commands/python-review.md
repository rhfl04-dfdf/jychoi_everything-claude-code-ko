...
[HIGH] 가변 기본 인자 (Mutable default argument)
파일: app/services/auth.py:18
이슈: 가변 기본 인자가 상태 공유를 유발함
```python
...
[MEDIUM] 누락된 type hints
파일: app/services/auth.py:25
이슈: 타입 어노테이션이 없는 공개 함수
```python
...
권장 사항: ❌ CRITICAL 이슈가 수정될 때까지 병합(merge) 차단
